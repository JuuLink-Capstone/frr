...
for TARGET in "${TARGETS[@]}"; do
    RESULT=$(fping -C 10 -q -p 200 -I $IFACE "$TARGET" 2>&1 | tail -1)
    LOST=$(echo "$RESULT" | tr ' ' '\n' | grep -c '^\-$')
    ...
    if [ -n "$RTTS" ]; then
        ...
        JITTER=$(echo "$RTTS" | awk '{sum+=$1; sumsq+=$1*$1; n++} END{if(n>1) printf "%.2f", sqrt(sumsq/n - (sum/n)^2); else print 0}')
    else
        AVG=9999
        JITTER=9999
    fi
